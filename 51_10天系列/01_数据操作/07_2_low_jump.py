...
def CheckBreakUp(df_daily, code):
  ...
  daily_limit_idx = np.where(diff > raise_limit)[0]
  # print(type(daily_limit_idx))
  if (daily_limit_idx.shape[0] < 1 or daily_limit_idx.shape[0] > 2):  # 1-2个涨停
  # if (daily_limit_idx.size == 0 or daily_limit_idx.shape[0] < 2):
    return False
